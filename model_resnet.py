...
def block_resnet(entree, filters, kernel_size, dropout=None, reduce=False):
    ...
    result=layers.Conv2D(filters, kernel_size, strides=1, padding='SAME')(entree)
    ...
    result=layers.Conv2D(filters, kernel_size, strides=stride, padding='SAME')(result)
        
    if entree.shape[-1]==filters:
        if reduce is True:
            shortcut=layers.Conv2D(filters, 1, strides=stride, padding='SAME')(entree)
        else:
            shortcut=entree
    else:
        shortcut=layers.Conv2D(filters, 1, strides=stride, padding='SAME')(entree)
...
def model(nbr_classes, nbr_attributs, nbr_boxes, nbr_cellule, nbr_cc=42):
    ...
    result=block_repeat(entree, 1*nbr_cc, [5, 3], dropout=0.3)
    result=block_resnet(result, 1*nbr_cc, 3, dropout=0.3, reduce=True)

    result=block_repeat(result, 2*nbr_cc, [3, 3, 3, 3], dropout=0.4)
    result=block_resnet(result, 2*nbr_cc, 3, dropout=0.4, reduce=True)

    result=block_repeat(result, 4*nbr_cc, [3, 3, 3, 3, 3, 3, 3, 3], dropout=0.5)
    result=block_resnet(result, 4*nbr_cc, 3, dropout=0.5, reduce=True)

    result=block_repeat(result, 8*nbr_cc, [3, 3, 3, 3, 3, 3, 3, 3], dropout=0.5)
